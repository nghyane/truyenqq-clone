...
import {
  MangaButton,
  MangaButtonFollowed,
  ChapterButtonFollowed,
  ChapterButton,
} from "@/views/components/Bookmark";
...
class BookmarkController {
  ...
  addBookmark = async ({
    ...
    return payload.template === "manga" ? (
      <MangaButtonFollowed manga_id={payload.id} />
    ) : (
      <ChapterButtonFollowed manga_id={payload.id} />
    );
  };
  ...
  removeBookmark = async ({
    ...
    return payload.template === "manga" ? (
      <MangaButton manga_id={payload.id} />
    ) : (
      <ChapterButton manga_id={payload.id} />
    );
  };
  ...
  getBookmarkStatus = async ({ verify, query, set }: VerifyContext) => {
    ...
    if (!session) {
      return query.template === "manga" ? (
        <MangaButton manga_id={new Number(query.id).valueOf()} />
      ) : (
        <ChapterButton manga_id={new Number(query.id).valueOf()} />
      );
    }
    ...
    return query.template === "manga" ? (
      <MangaButtonFollowed manga_id={new Number(query.id).valueOf()} />
    ) : (
      <ChapterButtonFollowed manga_id={new Number(query.id).valueOf()} />
    );
  };
  ...
  getBookmarks = async ({ verify, set, query }: VerifyContext) => {
    ...
    const mangas = bookmarks?.bookmarks.map(
      (bookmark) => bookmark.manga,
    ) as any;
