...
const Manga1001 = async () => {
    const BASE_URL = 'https://jmanga.info';
    ...
    const collectGenres = async (): Promise<string[]> => {
        ...
        const html = await fetch(`${BASE_URL}/list/`, fetchOptions).then(res => res.text());

        const $ = cheerio.load(html);

        const genres = $('.container .col-md-3 a.d-block').map((i, el) => {
            return $(el).text();
        }).get();
    ...
    const collectUrls = async (page: Number) => {
        ...
        const html = await fetch(`${BASE_URL}/page/${page}`, fetchOptions).then(res => res.text());

        if (!html) return [];

        const $ = cheerio.load(html);

        const urls = $('.container .card-body .text-center.text-white.text-shadow.p-2 a:nth-child(1)').map((i, el) => {
    ...
    const collectManga = async (url: string) => {
        ...
        const cover = $('img.aligncenter').attr('data-src') || $('img.aligncenter').attr('src');


        const tags = $('.container .text-left.text-white.text-shadow.small.p-2 a').map((i, el) => {
            return $(el).text();
        }).get();

        const genres = $('#primary .text-center.text-white.text-shadow.small.p-2 a').map((i, el) => {
            return $(el).text();
        }).get();
    ...
    const collectChapter = async (url: string) => {
        ...
        try {
            ...
            const matchResult = html.match(/'8HdOdbI','2069060yNuhPa','24168vYWUfr','(.*?)','join','match'/);
            const encrypted = matchResult ? matchResult[1] : null;

            function decryptString(key: string, encryptedString: { match: (arg0: RegExp) => any[]; }) {
                const stringToCharCodes = (str: string) => str.split('').map((char: string) => char.charCodeAt(0));
                const xorReducer = (accumulator: number, currentValue: number) => accumulator ^ currentValue;
                return encryptedString.match(/.{1,2}/g)
            ...
            const decryptedArray = decryptString(BASE_URL, encrypted! as { match: (arg0: RegExp) => any[]; }).split('","');

            if (decryptedArray.length > 1) {
                fs.appendFileSync(
                    'scraper.log', "Unkown encrypted chap: " + url + '\n'
                );
            ...
            fs.appendFileSync(
                'scraper.log', "Failed to decrypt chap: " + url + '\n'
            );
        ...
        const pages = $('.container-chapter-reader .card-wrap .image-vertical').map((i, el) => {
